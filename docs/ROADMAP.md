...
- ✅ **Phase 3 — four-化 metrics** (SHIPPED v1.28.0): `sgc metrics` computes all four 化 live from embedded contracts + compiled symbols (no baseline read at runtime); `metrics/metrics-baseline.yaml` is a dev/CI drift reference (doctor check `J`/`K`), and doctor check `M` gates the README scorecard line against live output. **For the current numbers read README's scorecard line or run `sgc metrics` — this file deliberately no longer restates them.** It restated them twice and was wrong both times: audit v1.31.8 P1-4 caught 自动化 still quoting 4/6 after the CE-arc stages joined the denominator at v1.29, and 智能化 sat at 11/23 here through v1.35.0's move to 13/23. Nothing gates this file, so a number copied into it is a number that will go stale — the same defect the v1.36.0 agent-description work exists to make unrepresentable, one document over. Spec/plan under `docs/superpowers/`.
...
- ~~**doctor `bundleParityCheck` is content-only** — it does not check file mode~~ → ✅ **CLOSED; the account was stale, not the code** (back-annotated 2026-07-15). The check reads the **git-recorded** mode — deliberately not the working-tree FS mode, which `build:cli` may already have chmod'd to 0o755 locally, masking the very drift we're hunting — and **fails** on 100644: `bundleParityCheck` in `src/commands/doctor.ts`, helper `bundleExecBitOk`. Its OK line reads `✓ committed bundle matches source rebuild (content + exec bit)`, and that helper's docblock names the v1.24.0 mode-drift snag as the motivation. (Named by symbol, not `file:line` — this bullet pinned `:406-414` / `:113-119` until v1.36.0 inserted check (O) above them and moved both.) Untracked / non-repo checkouts return `null` → skipped rather than failed.
