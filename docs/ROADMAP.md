...
- ✅ **Phase 2d — browse on the npm channel** (RESOLVED — *document the degradation as intended*): we deliberately do **not** ship the ~100 MB vendored `browse` binary on npm (it would bloat the ~1.7 MB package ~60×; `sgc doctor` check `E` keeps `plugins/` out of the npm `files` allowlist). Wiring `sgc qa` to drive the binary is **deferred on both channels** (`runQa` never builds a runner; the `SGC_QA_REAL` / `--browse` opt-in is reserved, read by no code; the real path is reachable today only via a programmatic injected `browseRunner` test seam), so by default `sgc qa` runs a stub returning `concern` (never rubber-stamps) on every channel. For real-browser QA today, use `gs:/browse` (optional interop). Documented across `docs/POSITIONING.md` "Vendored components", `README.md`, `plugins/sgc/CLAUDE.md`, `plugins/sgc/commands/qa.md`, `plugins/sgc/skills/qa/SKILL.md`, and the `sgc qa` stub runtime message. Actually wiring the real runner (env/flag → shell out to the binary) remains future work (the deferred real-browser implementation). **Roadmap Phase 0–3 + every Phase-2 item now documentation-complete; the real-browser runner + optional 2e (native parallel-subagents) + cross-cutting debt remain.**
...
- **2d. browse on the npm channel.** ✅ **RESOLVED** — *document the degradation as intended* (see Status above). Decision: do **not** ship the ~100 MB vendored `browse` binary on npm. Wiring `sgc qa` to drive it is deferred on both channels (stub by default — `concern`, never rubber-stamps; the `SGC_QA_REAL` / `--browse` opt-in is reserved, not yet wired); `gs:/browse` is the path for real-browser QA today. Documented across POSITIONING / README / plugin CLAUDE.md / commands/qa.md / skills/qa + the `sgc qa` stub runtime message. (Actually wiring the runner is future work.)
