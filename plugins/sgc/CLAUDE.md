...
## Implementation Status (v1.38.4 — C11 review follow-up: proxy-fetch now honors NO_PROXY + auto-bypasses loopback (canary localhost probes stop being forced through the proxy); two seam-default regression tests guard that openrouter/canary actually route through proxyAwareFetch; malformed HTTP(S)_PROXY fails fast with a named error; Node ≥ 18.17 synced across README + marketplace/plugin descriptions · prior: v1.38.3 — engines honesty: `engines.node` tightened `>=18` → `>=18.17` to match undici ^6's real floor (shipped in v1.38.2), so the declared minimum stops under-claiming · prior: v1.38.2 — audit-v1.37.0 remediation C11 (closes the audit): proxy-aware fetch — OpenRouter LLM requests + canary health checks now honor HTTP(S)_PROXY via undici's ProxyAgent, gated on the env var so the no-proxy path is unchanged (undici pinned ^6 for Node-18 support) · prior: v1.38.1 — audit-v1.37.0 remediation batch C: correctness cluster (fuse-plan severity-merge adopts the high-severity representative; canary throws on an unknown phase; helm/argo word-bounded; doctor skip rows state what was tested) + robustness (claude-cli reap escalates SIGTERM→SIGKILL after a grace) + C9 layering inversion fixed (dispatcher no longer imports commands, structurally guarded) + C10 refactor: state.ts god-module split into per-layer modules behind a re-export barrel, and runDoctor decomposed into per-check functions — behavior-preserving · prior: v1.38.0 — audit-v1.37.0 remediation batch A+B: `sgc ship` blocks an L2+ ship whose every code review is heuristic-only (no LLM), with a signed `--accept-degraded-review`/`--accepted-by` escape hatch recorded immutably in ship.md + captureDiff tri-state (large diff no longer collapses the review gate) + dedup empty-token fix (distinct knowledge no longer fused) + writeSolution read-modify-write locked, atomic lock primitive (no empty-file race) + streaming multibyte decode + output byte cap + classifier structural floor (keyword-free rewrites → L2) + fail-override requires a named signer + publish gate locks tag == package.json == plugin.json and runs dispatcher + eval · prior: v1.37.0 — GS-N arc 7/7 + Tier-1 sp absorb + L2 reviewer cluster wired + four-化 metrics + CLI UX hardening + QA/ship gate integrity + SGC_STATE_ROOT honored everywhere + LLM planner resilience + audit-v1.31.8 remediation: classifier heuristic floor in LLM mode, §3 stamp provenance, §13 Tier-2 signal pairing, argv-free prompts, agent-registry honesty gate + code-review follow-up: published artifact inside the cso gate, OpenAI/Stripe/Slack/Google/GitHub key coverage, doc-context tiering, §1 rejection on the event stream, reboot-proof run locks, per-executor agent descriptions + M5 description inversion: reviewer.security/tests LLM-backed (智能化 13/23), agent descriptions lead with capability, migration/infra registry files, O(n) matcher repaired + description derivation: the machine-owned half of 9 agent descriptions generated from code (agent-facts.ts), doctor check (O) fails on drift, sgc doctor --write-descriptions regenerates, matcher+trigger built from one term list per specialist + review follow-up: each clause pinned to its own def (a mis-mapped id no longer regenerates green), spawn caveat keyed to the hazard rather than the clause shape, severityOf returns Severity so a wrong arm is a compile error)
