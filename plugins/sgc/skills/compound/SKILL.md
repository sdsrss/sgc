...
- Automatically triggered by `sgc:janitor:compound` after `/ship`
...
#### Agent 4: `sgc:compound:prevention`
...
1. **Check dedup verdict** from `sgc:compound:related`:
...
## Problem Context
[from sgc:compound:context]

## Solution
[from sgc:compound:solution]

## Prevention
[from sgc:compound:prevention]

## Related
[from sgc:compound:related — links to similar solutions]
...
Write to `reviews/{task_id}/janitor/compound-decision.md`:
...
- **Dedup before write.** `sgc:compound:related` must run before any write to `solutions/`. No exceptions. Threshold is 0.85, not tunable.
