...
- ✅ **Phase 3 — four-化 metrics** (SHIPPED v1.28.0): `sgc metrics` computes 规范化 12/13 · 智能化 11/23 LLM-invokable · 自动化 5/9 · 高效化 1 step·node≥18 live from embedded contracts + compiled symbols (no baseline read at runtime); `metrics/metrics-baseline.yaml` is a dev/CI drift reference (doctor check `J`/`K`), and doctor check `M` gates the README scorecard line against live output. (自动化 was 4/6 pre-v1.29; the CE-arc stages joined the denominator when the knowledge loop was metered — audit v1.31.8 P1-4 found this line still quoting the old figure.) Spec/plan under `docs/superpowers/`.
