...
After successful ship, automatically dispatch `unified:janitor:compound` to decide whether to extract knowledge.
...
The janitor's decision (including skips) MUST be logged to `reviews/{task_id}/janitor/decision.md`. Silent skips are forbidden (Invariant #6).
