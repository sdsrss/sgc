...
> - **v1.33.0**（`f20feb5`）：测试 → **1372 pass / 38 skip / 0 fail**（较审核基线 1269 **+103 回归测试**）· `sgc doctor` **66 OK / 0 warn / 0 fail**（新增检查 N）· `npm audit` **0** · CI 双绿 · npm 包 **529KB → 296KB（-44%）**、文件数 90 → 5 · 从 registry 实装 v1.33.0 验证：`src/` 确已不随包发出、doctor 34 OK / 0 fail。
>   > **⚠️ 数字更正（M4）**：本行原写 "1410 pass … +141 回归测试"，错。1410 是 bun 的 `Ran N tests`，不是 pass 数。三个 commit 逐一实测后的真值：1269 → 1339（v1.32.0）→ 1372（v1.33.0），M3 单独 **+33** 而非 +71。详见路线图。
>
