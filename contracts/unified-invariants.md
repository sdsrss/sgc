...
The dispatcher validates every file write against `unified-state.schema.yaml` before committing. A write that fails validation is rejected with the validation error surfaced to the calling subagent. Subagents may retry with corrected output; they may not disable validation.
...
- Invariant §1 is enforced by `unified-capabilities.yaml` scope token `read:solutions` (forbidden_for list) and by the empty `solutions` row in the permission matrix for `/review` and `/qa`.
- Invariant §2 is enforced by the `editable_after_creation: false` field on `decisions.intent` and `decisions.ship` in `unified-state.schema.yaml`.
- Invariant §3 is enforced by the `dedup` block in `solutions` section of `unified-state.schema.yaml`, plus a dispatcher check.
